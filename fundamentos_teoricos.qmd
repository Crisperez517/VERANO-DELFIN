...
Sea la ecuación diferencial estocástica continua $dX(t) = b(t, X(t))dt + \sigma(t, X(t))dW(t)$ sujeta a las condiciones de existencia y unicidad. Si los coeficientes de deriva $b$ y de difusión $\sigma$ son continuamente diferenciables y satisfacen las condiciones de Lipschitz y crecimiento lineal, entonces el esquema de Euler-Maruyama converge fuertemente con orden:
...
La demostración de este orden de convergencia estricto se fundamenta en que el incremento del movimiento Browniano domina algebraicamente sobre el incremento del tiempo determinista, dado que su escala de variación es proporcional a $\sqrt{\Delta t}$.
...
El incremento en la velocidad de convergencia fuerte se logra al incorporar de forma analítica el término correspondiente a la integral estocástica iterada $\int_{t_n}^{t_{n+1}} \int_{t_n}^s dW(u) dW(s) = \frac{1}{2} \left[ (\Delta W_n)^2 - \Delta t \right]$, la cual modela la curvatura interna del ruido estocástico dentro del intervalo de simulación.
