...
### Proceso Estocástico
Sea $(\Omega, \mathcal{F}, \mathbb{P})$ un espacio de probabilidad y sea $\mathcal{S} \subseteq \mathbb{R}^d$ un espacio muestral dotado de su respectiva $\sigma$-álgebra de Borel. Un proceso estocástico es una colección de variables aleatorias $\{X(t)\}_{t \in \mathcal{T}}$ donde cada $X(t): \Omega \to \mathcal{S}$ es una función medible para cada parámetro indexado $t$ perteneciente a un conjunto de índices o dominio temporal $\mathcal{T}$.
:::

Dependiendo de la naturaleza algebraica del dominio indexado $\mathcal{T}$, los procesos estocásticos se clasifican formalmente en dos grandes vertientes estructurales:

1. **Procesos estocásticos a tiempo discreto:** Ocurren cuando el conjunto de índices es numerable, típicamente denotado por $\mathcal{T} = \mathbb{N}_0 = \{0, 1, 2, \dots\}$ o $\mathcal{T} = \mathbb{Z}$. En este escenario, el sistema evoluciona mediante transiciones discretas en pasos fijos discretizados.
2. **Procesos estocásticos a tiempo continuo:** Ocurren cuando el conjunto indexado es un intervalo continuo real, típicamente expresado como $\mathcal{T} = [0, \infty)$ o en un horizonte acotado $\mathcal{T} = [0, T]$. Esta estructura es la adecuada para modelar fenómenos físicos continuos, como la transferencia térmica.
...
::: {#def-trayectoria}
### Trayectoria (Función Muestral)
Para cada elemento fijo u omisión puntual del espacio muestral $\omega \in \Omega$, la función determinista que mapea el dominio temporal al espacio de estados, definida por $t \mapsto X(t, \omega)$ para todo $t \in \mathcal{T}$, se denomina trayectoria, camino muestral o función muestral del proceso estocástico.
:::

Las propiedades analíticas de un proceso estocástico, tales como la continuidad, la diferenciabilidad y la presencia de discontinuidades finitas, se definen estrictamente a partir del comportamiento matemático de sus trayectorias [@platen2010numerical]. Los componentes aleatorios de nuestro sistema se sustentan en tres procesos estocásticos específicos a tiempo continuo.
...
### Proceso de Wiener
Un proceso estocástico continuo en el tiempo $\{W(t)\}_{t \ge 0}$ definido sobre un espacio de probabilidad filtrado $(\Omega, \mathcal{F}, \{\mathcal{F}_t\}, \mathbb{P})$ es un movimiento Browniano estándar o proceso de Wiener si cumple con las siguientes propiedades:
...
3. Posee incrementos estacionarios gaussianos tales que para todo $t > s \ge 0$, el incremento $W(t) - W(s) \sim \mathcal{N}(0, t-s)$.
4. Sus trayectorias son continuas con probabilidad 1.
:::

El término $dW(t) = W(t+dt) - W(t)$ representa el diferencial del proceso de Wiener. Este denota una perturbación gaussiana elemental en un intervalo infinitesimal $dt$, caracterizada matemáticamente por una media $\mathbb{E}[dW(t)] = 0$ y una varianza $\mathbb{E}[(dW(t))^2] = dt$. Para una revisión detallada sobre las propiedades de medida y construcciones analíticas de este proceso, se sugiere consultar a @platen2010numerical.
...
::: {#def-proceso-poisson}
### Proceso de Poisson
Un proceso estocástico lineal continuo en el tiempo $\{N(t)\}_{t \ge 0}$ que toma valores enteros en el espacio de estados $\mathcal{S} = \mathbb{N}_0$ es un proceso de Poisson homogéneo con parámetro de intensidad constante $\lambda > 0$ si satisface las siguientes condiciones:

1. $N(0) = 0$ con probabilidad 1.
2. Posee incrementos independientes; de tal manera que para cualquier secuencia temporal finita $0 \le t_1 < t_2 < \dots < t_n$, las variables aleatorias discretas $N(t_2)-N(t_1), \dots, N(t_n)-N(t_{n-1})$ son mutuamente independientes en sentido probabilístico.
