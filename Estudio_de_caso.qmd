...
![Refrigerador institucional para vacunas Haier HBC-150 (Ice-Lined Refrigerator). Este equipo cuenta con certificación PQS de la OMS y masa térmica incorporada para garantizar la red de frío durante cortes prolongados de energía eléctrica.](haier_hbc150.jpg){#fig-haier-hbc150 width="70%" fig-align="center"}

Para reflejar el comportamiento de este equipo bajo el estándar normativo de la OMS, calibramos cada coeficiente de la @eq-ede-operativa con base en la física del enfriamiento:
...
* **Temperatura de Consigna ($\theta_{set}$)**: Se establece en **$5^\circ \text{C}$**, el centro ideal del rango normativo de seguridad (2°C a 8°C).
* **Velocidad de Enfriamiento ($\kappa_{on}$)**: Representa la capacidad de potencia del motor para recuperar la temperatura. Se calibra en $\kappa_{on} = 0.1188$, una velocidad rápida propia de la compresión activa.
* **Ruido del Compresor ($\sigma_{on}$)**: El motor prende y apaga intermitentemente para mantener los 5°C. Para que el 99.7% del tiempo las fluctuaciones se mantengan en un rango de $\pm 3^\circ \text{C}$ (aplicando la regla empírica de 3 sigmas para la varianza estacionaria de un proceso Ornstein-Uhlenbeck), se calcula una desviación de $\sigma_{on} = \sqrt{0.2376} \approx 0.487$.

### Régimen 2: Falla Eléctrica ($\alpha(t) = 2$)
Se activa cuando el Proceso de Poisson dispara un apagón y el enfriamiento activo se detiene.

* **Temperatura Exterior ($T_{amb}(t)$)**: La temperatura ambiente actúa como el atractor del sistema. Dado que en Tuxtla Gutiérrez el calor oscila drásticamente a lo largo del día, esta variable se modela computacionalmente como una función senoidal que alcanza su pico máximo diario a las 15:00 hrs. Para nuestra calibración analítica del peor escenario (estiaje), evaluamos este atractor en un máximo absoluto de **$43^\circ \text{C}$**. En este escenario, el término de deriva $(T_{amb} - T(t))$ se vuelve enorme y genera un calentamiento fuertemente acelerado.
* **Velocidad de Pérdida Térmica ($\kappa_{off}$)**: Los refrigeradores OMS cuentan con paquetes de hielo en su interior que actúan como "masa térmica". El equipo Haier garantiza un *Holdover Time* (tiempo de autonomía) de 63.8 horas a 43°C antes de rebasar los 8°C. Usando la solución analítica de la Ley de Enfriamiento de Newton desde 5°C a 8°C:
  $$8 = 43 + (5 - 43) e^{-\kappa_{off} (63.8)}$$
  Al despejar, obtenemos un valor sumamente bajo de $\kappa_{off} \approx 0.00128$. Esta transición de un parámetro rápido ($\kappa_{on}$) a uno muy lento ($\kappa_{off}$) es lo que justifica el uso de EDEs con conmutación de régimen.
...
A diferencia de los cortes de luz, las aperturas de puerta son eventos provocados activamente por el personal de enfermería. 

* **Tasa de Aperturas**: Se modelan con un Proceso de Poisson Homogéneo tradicional que ocurre exclusivamente durante los horarios operativos del centro de salud.
* **Impacto de la Apertura ($\gamma$)**: Asignamos un incremento térmico inmediato de **$+0.5^\circ \text{C}$** por evento. Esta es la perturbación inelástica instantánea que rompe la continuidad de la temperatura y "roba" el frío del equipo de forma acumulativa antes de un posible apagón.