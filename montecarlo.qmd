...
Para evaluar la confiabilidad de la red de frío frente a la volatilidad del suministro eléctrico y las condiciones climáticas de Tuxtla Gutiérrez, Chiapas, desarrollamos un simulador estocástico modular en Python enfocado en el refrigerador institucional Haier HBC-150. Debido a la interacción de múltiples procesos aleatorios intermitentes (cortes de energía de CFE, aperturas de puerta y variaciones extremas de temperatura), el sistema no posee una solución matemática analítica cerrada. En consecuencia, recurrimos a la integración numérica de trayectorias acoplada al método de Montecarlo.

> **Nota sobre Reproducibilidad:** Con el fin de mantener este documento con un formato académico limpio y legible, hemos omitido el código fuente de programación en el cuerpo del texto. El script completo, estructurado en celdas de ejecución con semillas de aleatoriedad controladas para su total replicabilidad, se encuentra alojado y documentado detalladamente en el [repositorio público de GitHub de este proyecto](https://github.com/Crisperez517/VERANO-DELFIN/blob/3a3f49b27bca9671e71b40c738dec826c8eb30cc/VERANO_DELFIN_SIMULACION_CODIGO_DE_PYTHON.ipynb) [@CPG2026Delfin].

A continuación, exponemos detalladamente el diseño metodológico de la simulación estructurado según el orden lógico de las celdas del algoritmo computacional, seguido de la discusión profunda de los resultados macroscópicos obtenidos.

## 1. Procesamiento Temporal y Entorno Climático Determinista

El primer módulo de la simulación constituye el motor temporal del sistema. Definimos una malla de tiempo continua para cubrir un año completo ($366$ días, contemplando el año bisiesto 2024). Para capturar microfluctuaciones de la temperatura interna y la naturaleza instantánea de los eventos logísticos, seleccionamos un paso de integración discreto de exactamente 5 minutos ($\Delta t = 5/60$ horas), lo que genera un total de $105,408$ iteraciones por cada trayectoria anual simulada.

Para acoplar la logística humana, programamos un calendario interno que identifica los días hábiles (lunes a viernes) y los fines de semana (sábados y domingos), debido a que la extracción de vacunas y las consultas clínicas ocurren exclusivamente en jornadas laborales.

En el aspecto ambiental, cargamos la base de datos meteorológicos reales de Tuxtla Gutiérrez. No obstante, las bases de datos públicas solo proveen la temperatura promedio diaria ($T_{media}$), ocultando el estrés térmico intradiario. Para solucionar esto sin saturar computacionalmente el programa con archivos horarios masivos, expandimos el vector diario a la escala microscópica de 5 minutos mediante una asignación de piso matemático y sobrepusimos una función armónica (onda senoidal) con una amplitud térmica calibrada de $7^\circ\text{C}$ (equivalente a una oscilación típica de 14°C entre la madrugada y la tarde chiapaneca). Desfasamos el argumento del coseno por 15 horas para forzar matemáticamente que el pico absoluto de calor golpee al equipo a las 15:00 horas de cada día simulado:

$$
T_{amb}(t) = T_{media}(t) + 7.0 \cdot \cos\left(2\pi \frac{t - 15}{24}\right)
$$

La validación visual de este entorno climático se presenta en la @fig-clima-anual, que exhibe el comportamiento macroscópico anual, y en la @fig-clima-zoom, que demuestra la consistencia del ciclo diurno continuo.

![Temperatura ambiente continua simulada para Tuxtla Gutiérrez a lo largo de un año completo, contrastada con los promedios diarios de la base de datos meteorológica original.](temp_anual_tuxtla.png){#fig-clima-anual}

![Acercamiento a una ventana de 5 días de simulación climática (Mayo), donde se observa el comportamiento de la oscilación senoidal intradiaria y la ubicación exacta de los picos térmicos a las 15:00 horas.](temp_zoom_tuxtla.png){#fig-clima-zoom}

## 2. Parametrización Física, Cinética y Estocástica de la Red

Una vez construido el entorno temporal y climático, procedemos a inicializar las constantes que gobiernan las fuerzas físicas del refrigerador Haier HBC-150 y las propiedades biológicas de las vacunas.

### Parámetros Termodinámicos y Cinéticos del Equipo
Asignamos los coeficientes térmicos constantes previamente estimados para el modelo de Ornstein-Uhlenbeck (EDE). Cuando el equipo cuenta con electricidad, la tasa de abatimiento térmico se fija en $\kappa_{on} = 0.1188\,\text{hr}^{-1}$ con una volatilidad interna de los ventiladores de $\sigma_{on} = 0.325$. Ante un escenario de falla eléctrica, el compresor se apaga y el aislamiento de alta densidad del equipo (reforzado por sus paquetes de hielo internos) frena la transferencia de calor, lo que se modela con una inercia pasiva sumamente lenta de $\kappa_{off} = 0.00178\,\text{hr}^{-1}$ y un ruido por microfiltraciones de aire de $\sigma_{off} = 0.05$. La temperatura objetivo del termostato se establece en $\theta = 4.0^\circ\text{C}$ y el equipo arranca estabilizado en esta misma condición inicial ($T_0 = 4.0^\circ\text{C}$).

Para modelar la degradación biológica, configuramos las constantes de la ecuación de Arrhenius para un biológico termosensible genérico (perfil biológico representativo de vacunas vivas atenuadas como Sarampión o Poliomielitis). La energía de activación crítica se establece en $E_a = 85,000\,\text{J/mol}$ ($85\,\text{kJ/mol}$), la constante universal de los gases en $R = 8.314\,\text{J/(mol}\cdot\text{K)}$ y el factor de frecuencia preexponencial empírico en $A = 1.2 \times 10^{14}$. 
